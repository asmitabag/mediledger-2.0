...
describe("MedicalRecord Contract", function () {
    ...
    // Test data
    const testPreimage = hashToField("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
    ...
    before(async function () {
        ...
        if (!fs.existsSync(wasmPath) || !fs.existsSync(zkeyPath)) {
            ...
            console.log("Run 'npm run build:circuit' and 'npm run setup:zkey' first.");
    ...
    describe("Deployment", function () {
        it("Should set the correct verifier address", async function () {
            expect(await medicalRecord.verifier()).to.equal(await verifier.getAddress());
        });

        it("Should grant admin role to deployer", async function () {
            const DEFAULT_ADMIN_ROLE = ethers.ZeroHash; // bytes32(0)
            expect(await medicalRecord.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.be.true;
        });

        it("Should not grant healthcare provider role to deployer by default", async function () {
            const HEALTHCARE_PROVIDER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("HEALTHCARE_PROVIDER_ROLE"));
            expect(await medicalRecord.hasRole(HEALTHCARE_PROVIDER_ROLE, owner.address)).to.be.false;
        });
    });
    ...
    describe("Role Management", function () {
        it("Should allow admin to add healthcare provider", async function () {
            ...
            const HEALTHCARE_PROVIDER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("HEALTHCARE_PROVIDER_ROLE"));
            expect(await medicalRecord.hasRole(HEALTHCARE_PROVIDER_ROLE, healthcare1.address)).to.be.true;
        });
        ...
        it("Should allow admin to remove healthcare provider", async function () {
            ...
            const HEALTHCARE_PROVIDER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("HEALTHCARE_PROVIDER_ROLE"));
            expect(await medicalRecord.hasRole(HEALTHCARE_PROVIDER_ROLE, healthcare1.address)).to.be.false;
        });

        it("Should not allow non-admin to add healthcare provider", async function () {
            await expect(
                medicalRecord.connect(unauthorized).addHealthcareProvider(healthcare1.address)
            ).to.be.revertedWithCustomError(medicalRecord, "AccessControlUnauthorizedAccount");
        });
    });
    ...
    describe("Proof Verification and Commitment", function () {
        ...
        it("Should reject commitment from unauthorized account", async function () {
            ...
            await expect(
                medicalRecord.connect(unauthorized).commitRecord(
                    mockProof.a,
                    mockProof.b,
                    mockProof.c,
                    mockProof.input
        ...
        it("Should validate contract parameters correctly", async function () {
            ...
            const tx = await medicalRecord.connect(healthcare1).commitRecord(
                mockProof.a,
                mockProof.b,
                mockProof.c,
                mockProof.input
            ...
            expect(commitments[0]).to.equal(ethers.zeroPadValue(ethers.toBeHex(mockProof.input[0]), 32));
        });
        ...
        it("Should generate and verify real proof if circuit is available", async function () {
            ...
            if (fs.existsSync(wasmPath) && fs.existsSync(zkeyPath)) {
                ...
                const commitments = await medicalRecord.getCommitments(healthcare1.address);
                expect(commitments.length).to.equal(1);
                expect(commitments[0]).to.equal(ethers.zeroPadValue(ethers.toBeHex(publicSignals[0]), 32));
        ...
        it("Should prevent duplicate commitments", async function () {
            ...
            await medicalRecord.connect(healthcare1).commitRecord(
                mockProof.a,
                mockProof.b,
                mockProof.c,
                mockProof.input
            ...
            await expect(
                medicalRecord.connect(healthcare1).commitRecord(
                    mockProof.a,
                    mockProof.b,
                    mockProof.c,
                    mockProof.input
    ...
    describe("Commitment Queries", function () {
        beforeEach(async function () {
            ...
            const testPreimage2 = hashToField("0xabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdef");
            const mockProof2 = {
                a: [1, 2],
                b: [[1, 2], [3, 4]],
                c: [5, 6],
        ...
        it("Should return correct commitment count", async function () {
            ...
            const count2 = await medicalRecord.getCommitmentCount(healthcare2.address);
    ...
    describe("Events", function () {
        describe("Events", function () {
            it("Should emit RecordCommitted event with correct parameters", async function () {
                ...
                const tx = medicalRecord.connect(healthcare1).commitRecord(
                    mockProof.a,
                    mockProof.b,
                    mockProof.c,
                    mockProof.input
        ...
        describe("Edge Cases", function () {
            ...
            it("Should maintain separate commitment lists for different providers", async function () {
                ...
                const commitment1 = hashToField("0x1111111111111111111111111111111111111111111111111111111111111111");
                const commitment2 = hashToField("0x2222222222222222222222222222222222222222222222222222222222222222");
                ...
                await medicalRecord.connect(healthcare2).commitRecord(
                    mockProof2.a,
                    mockProof2.b,
                    mockProof2.c,
                    mockProof2.input
    ...
    describe("Advanced Proof Validation", function () {
        ...
        it("Should properly validate field constraints with real circuit", async function () {
            ...
            if (fs.existsSync(wasmPath) && fs.existsSync(zkeyPath)) {
                ...
                const invalidPreimage = "21888242871839275222246405745257275088548364400416034343698204186575808495618"; // > field size
                ...
                try {
                    ...
                    // This is expected - circuit should reject invalid field elements
